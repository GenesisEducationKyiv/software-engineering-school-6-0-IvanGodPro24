...
- **Запити до GitHub API:** Якщо ми маємо 1000 унікальних репозиторіїв у базі і перевіряємо їх кожні 10 хвилин: `1000 * (60/10) * 24 = 144,000 запитів/день` (або 6,000 запитів/годину).
  ...
  > Оскільки 99% запитів не будуть знаходити кожен раз нові релізи, вони не будуть списуватися з ліміту GitHub, що дозволяє системі стабільно працювати навіть при такому навантаженні.
...
graph TD
    ...
    Scanner -- "3. Cache version tag" --> RedisQueue
