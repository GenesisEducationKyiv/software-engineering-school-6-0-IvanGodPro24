...
* Контракт менш строгий для service-to-service взаємодії.
* Request/response типи підтримуються вручну через TypeScript DTO та OpenAPI.
* Помилки передаються як HTTP responses, але всередині між сервісами немає RPC-level статусів і deadline semantics.
